...
import { UserPlus, GraduationCap, School, Mail, User, UtensilsCrossed } from "lucide-react";
import { z } from "zod";
...
const applicantSchema = z.object({
    ...
    useUserEmail: z.boolean().default(false)
}).refine((data) => {
...
export function AddApplicantModal({ isOpen, onClose, userEmail }: AddApplicantModalProps) {
    ...
    return (
        ...
        >
            <ModalContent className="w-full max-h-[85vh]">
                {(onClose) => (
                    <Form className="w-full flex flex-col overflow-hidden" onSubmit={handleSubmit(onSubmit)}>
                        <ModalHeader className="flex items-center w-full rounded-t-xl gap-3 bg-linear-to-r from-pink-100 to-pink-200 text-gray-800">
                            ...
                            <h2 className="text-2xl font-bold">เพิ่มข้อมูลผู้สมัคร</h2>
                        </ModalHeader>
                                                    ...
                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                                        {prefixes.map((prefix) => (
                                                            <Button
                                                                ...
                                                                className={selectedPrefix === prefix ? "bg-pink-400 text-white font-medium" : "font-medium"}
                                                            >
                                                                {prefix}
                                                            </Button>
                                                        ))}
                                                    ...
                                                    <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                                                        {educationLevels.map((level) => (
                                                            <Button
                                                                key={level}
                                                                type="button"
                                                                variant={selectedEducationLevel === level ? "solid" : "bordered"}
                                                                onPress={() => field.onChange(level)}
                                                                className={selectedEducationLevel === level ? "bg-pink-400 text-white font-medium" : "font-medium"}
                                                            >
                                                                {level}
                                                            </Button>
                                                        ))}
